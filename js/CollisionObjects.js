enemyMountain = function(ctx,h,w) {
	...
	this.canvas = ctx;
	this.line = [1,2,2,3,3,4,4,6,6,5,5,3,3,3,4,4,5,5,6,6,3,2,2,6,6,2,2,3,3,2,2,2,2,1];
	...
	this.boxTest = function(cPoint,bPoints) {
		...
		x1 = (bPoints[0][0]<bPoints[1][0])? bPoints[0][0]:bPoints[1][0];
		y1 = (bPoints[0][1]<bPoints[1][1])? bPoints[0][1]:bPoints[1][1];
		x2 = (bPoints[0][0]>bPoints[1][0])? bPoints[0][0]:bPoints[1][0];
	...
	this.collisionCheck = function(cPoints) {
		for(var c=0;c<cPoints.length;c++) {
			sP = Math.floor(cPoints[c][0]/this.mvWight);
			cx = cPoints[c][0]; cy = cPoints[c][1];
			x1 = sP*this.mvWight;
			y1 = this.lineToHeight(sP);
			x2 = (sP+1)*this.mvWight;
			y2 = this.lineToHeight(sP+1);
...
$(document).ready(function() {
	...
	test = new enemyMountain(ctx,350,800);
});
